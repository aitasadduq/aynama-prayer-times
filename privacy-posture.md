...
- **v1:** zero network calls after install. Zero.
  - **Android v1:** the app itself makes no network calls; its manifest doesn't declare `INTERNET`. The platform geocoder, Auto Backup and the Play services watch sync can still send data off the device on its behalf (see Android equivalents above).
...
- [ ] Disclose the watch sync (profiles through Google Play services) in the privacy page and the Data Safety form.
